...
def get_current_user():
    ...
    #user = User.query.filter(User.name == 'user20').first()
...
def guest_login():
    ...
    #modify code for client cert auth
    ...
    if s_dn:
        ...
        user = User.query.filter(User.email == 'Guest').first()
        if not user:
            user = User(
